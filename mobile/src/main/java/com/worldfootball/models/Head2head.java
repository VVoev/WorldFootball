...
public class Head2head implements Parcelable {
	...
	public static Head2head parse(Cursor head2headCursor) {
		...
		if (head2headCursor.moveToFirst()) {
			int col_fixture_id = head2headCursor.getColumnIndex(DbConstants.FIXTURE_ID);
			int col_count = head2headCursor.getColumnIndex(DbConstants.COUNT);
			int col_time_frame_start = head2headCursor.getColumnIndex(DbConstants.TIME_FRAME_START);
			int col_time_frame_end = head2headCursor.getColumnIndex(DbConstants.TIME_FRAME_END);
			int col_home_team_wins = head2headCursor.getColumnIndex(DbConstants.HOME_TEAM_WINS);
			int col_away_team_wins = head2headCursor.getColumnIndex(DbConstants.AWAY_TEAM_WINS);
			int col_draws = head2headCursor.getColumnIndex(DbConstants.DRAWS);
