...
public class Scores implements Parcelable {
	...
	public static Scores parse(Cursor scoresCursor) {
		...
		if (scoresCursor.moveToFirst()) {
			int col_soccerSeasonId = scoresCursor.getColumnIndex(DbConstants.SOCCER_SEASON_ID);
			int col_rank = scoresCursor.getColumnIndex(DbConstants.RANK);
			int col_team = scoresCursor.getColumnIndex(DbConstants.TEAM);
			int col_teamId = scoresCursor.getColumnIndex(DbConstants.TEAM_ID);
			int col_playedGames = scoresCursor.getColumnIndex(DbConstants.PLAYED_GAMES);
			int col_crestURI = scoresCursor.getColumnIndex(DbConstants.CREST_URI);
			int col_points = scoresCursor.getColumnIndex(DbConstants.POINTS);
			int col_goals = scoresCursor.getColumnIndex(DbConstants.GOALS);
			int col_goalAgainst = scoresCursor.getColumnIndex(DbConstants.GOAL_AGAINST);
			int col_goalDifference = scoresCursor.getColumnIndex(DbConstants.GOAL_DIFFERENCE);
			scores = new Scores();
