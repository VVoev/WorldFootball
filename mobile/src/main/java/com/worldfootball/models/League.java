...
public class League implements Parcelable {
	...
	public static League parse(Cursor leagueCursor) {
		...
		if (leagueCursor.moveToFirst()) {
			int col_id = leagueCursor.getColumnIndex(DbConstants.ID);
			int col_caption = leagueCursor.getColumnIndex(DbConstants.CAPTION);
			int col_league = leagueCursor.getColumnIndex(DbConstants.LEAGUE);
			int col_year = leagueCursor.getColumnIndex(DbConstants.YEAR);
			int col_currentMatchday = leagueCursor.getColumnIndex(DbConstants.CURRENT_MATCHDAY);
			int col_numberOfMatchdays = leagueCursor.getColumnIndex(DbConstants.NUMBER_OF_MATCHDAYS);
			int col_numberOfTeams = leagueCursor.getColumnIndex(DbConstants.NUMBER_OF_TEAMS);
			int col_numberOfGames = leagueCursor.getColumnIndex(DbConstants.NUMBER_OF_GAMES);
			int col_lastUpdated = leagueCursor.getColumnIndex(DbConstants.LAST_UPDATED);
			league = new League();
	...
	public static ArrayList<League> parseArray(Cursor leaguesCursor) {
		if (leaguesCursor.moveToFirst()) {
			int col_id = leaguesCursor.getColumnIndex(DbConstants.ID);
			int col_caption = leaguesCursor.getColumnIndex(DbConstants.CAPTION);
			int col_league = leaguesCursor.getColumnIndex(DbConstants.LEAGUE);
			int col_year = leaguesCursor.getColumnIndex(DbConstants.YEAR);
			int col_currentMatchday = leaguesCursor.getColumnIndex(DbConstants.CURRENT_MATCHDAY);
			int col_numberOfMatchdays = leaguesCursor.getColumnIndex(DbConstants.NUMBER_OF_MATCHDAYS);
			int col_numberOfTeams = leaguesCursor.getColumnIndex(DbConstants.NUMBER_OF_TEAMS);
			int col_numberOfGames = leaguesCursor.getColumnIndex(DbConstants.NUMBER_OF_GAMES);
			int col_lastUpdated = leaguesCursor.getColumnIndex(DbConstants.LAST_UPDATED);
